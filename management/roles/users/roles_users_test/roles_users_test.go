...
func TestRolesUsersListWithWireMock(
	...
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/roles/{id}/users")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"next": "next", "users": []interface{}{map[string]interface{}{"user_id": "user_id", "picture": "picture", "name": "name", "email": "email"}}},
...
func TestRolesUsersAssignWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/roles/{id}/users")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
