...
func TestGuardianPoliciesListWithWireMock(
	...
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/guardian/policies")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			[]interface{}{"all-applications"},
...
func TestGuardianPoliciesSetWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Put(gowiremock.URLPathTemplate("/guardian/policies")).WithBodyPattern(gowiremock.MatchesJsonSchema("{}", "V202012")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			[]interface{}{"all-applications"},
		).WithStatus(http.StatusOK),
	)
