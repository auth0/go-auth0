...
func TestGuardianEnrollmentsCreateTicketWithWireMock(
	...
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/guardian/enrollments/ticket")).WithBodyPattern(gowiremock.MatchesJsonSchema(`{
		...
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"ticket_id": "ticket_id", "ticket_url": "ticket_url"},
...
func TestGuardianEnrollmentsGetWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/guardian/enrollments/{id}")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"id": "id", "status": "pending", "name": "name", "identifier": "identifier", "phone_number": "phone_number", "enrolled_at": "enrolled_at", "last_auth": "last_auth"},
		).WithStatus(http.StatusOK),
	)
	err := WireMockClient.StubFor(stub)
	require.NoError(t, err, "Failed to create WireMock stub")
...
func TestGuardianEnrollmentsDeleteWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Delete(gowiremock.URLPathTemplate("/guardian/enrollments/{id}")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
