...
func TestGuardianFactorsListWithWireMock(
	...
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/guardian/factors")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			[]interface{}{map[string]interface{}{"enabled": true, "trial_expired": true, "name": "push-notification"}},
...
func TestGuardianFactorsSetWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Put(gowiremock.URLPathTemplate("/guardian/factors/{name}")).WithPathParam(
		"name",
		gowiremock.Matching("push-notification"),
