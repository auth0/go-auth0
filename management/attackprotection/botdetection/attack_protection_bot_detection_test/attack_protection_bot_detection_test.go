...
package attack_protection_bot_detection_test
...
func TestAttackProtectionBotDetectionGetWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/attack-protection/bot-detection")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"bot_detection_level": "low", "challenge_password_policy": "never", "challenge_passwordless_policy": "never", "challenge_password_reset_policy": "never", "allowlist": []interface{}{"allowlist"}, "monitoring_mode_enabled": true},
	...
	_, invocationErr := client.AttackProtection.BotDetection.Get(
		context.TODO(),
	)
...
func TestAttackProtectionBotDetectionUpdateWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Patch(gowiremock.URLPathTemplate("/attack-protection/bot-detection")).WithBodyPattern(gowiremock.MatchesJsonSchema(`{
	...
	_, invocationErr := client.AttackProtection.BotDetection.Update(
		context.TODO(),
		request,
	)
