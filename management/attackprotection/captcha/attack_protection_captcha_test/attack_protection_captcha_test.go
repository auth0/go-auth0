...
package attack_protection_captcha_test
...
func TestAttackProtectionCaptchaGetWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/attack-protection/captcha")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"active_provider_id": "active_provider_id"},
	...
	_, invocationErr := client.AttackProtection.Captcha.Get(
		context.TODO(),
	)
...
func TestAttackProtectionCaptchaUpdateWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Patch(gowiremock.URLPathTemplate("/attack-protection/captcha")).WithBodyPattern(gowiremock.MatchesJsonSchema(`{
		...
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"policy": "high_risk", "passwordless_policy": "high_risk", "password_reset_policy": "high_risk", "selected": "auth0", "providers": map[string]interface{}{"auth0_v2": map[string]interface{}{"failOpen": true}, "recaptcha_v2": map[string]interface{}{"siteKey": "siteKey", "secret": "secret"}, "recaptcha_enterprise": map[string]interface{}{"siteKey": "siteKey", "apiKey": "apiKey", "projectId": "projectId"}, "hcaptcha": map[string]interface{}{"siteKey": "siteKey", "secret": "secret"}, "friendly_captcha": map[string]interface{}{"siteKey": "siteKey", "secret": "secret"}, "arkose": map[string]interface{}{"siteKey": "siteKey", "secret": "secret", "clientSubdomain": "clientSubdomain", "verifySubdomain": "verifySubdomain", "failOpen": true}}, "allowlist": []interface{}{"allowlist"}},
		).WithStatus(http.StatusOK),
	)
	err := WireMockClient.StubFor(stub)
	require.NoError(t, err, "Failed to create WireMock stub")
	...
	_, invocationErr := client.AttackProtection.Captcha.Update(
		context.TODO(),
		request,
	)
