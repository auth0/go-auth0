...
func TestKeysSigningListWithWireMock(
	...
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/keys/signing")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			[]interface{}{map[string]interface{}{"kid": "kid", "cert": "cert", "pkcs7": "pkcs7", "current": true, "next": true, "previous": true, "current_since": "current_since", "current_until": "current_until", "fingerprint": "fingerprint", "thumbprint": "thumbprint", "revoked": true, "revoked_at": "revoked_at"}},
...
func TestKeysSigningRotateWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/keys/signing/rotate")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"cert": "cert", "kid": "kid"},
		).WithStatus(http.StatusOK),
	)
	err := WireMockClient.StubFor(stub)
	require.NoError(t, err, "Failed to create WireMock stub")
...
func TestKeysSigningGetWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/keys/signing/{kid}")).WithPathParam(
		"kid",
		gowiremock.Matching("kid"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"kid": "kid", "cert": "cert", "pkcs7": "pkcs7", "current": true, "next": true, "previous": true, "current_since": "current_since", "current_until": "current_until", "fingerprint": "fingerprint", "thumbprint": "thumbprint", "revoked": true, "revoked_at": "revoked_at"},
		).WithStatus(http.StatusOK),
	)
	err := WireMockClient.StubFor(stub)
	require.NoError(t, err, "Failed to create WireMock stub")
...
func TestKeysSigningRevokeWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Put(gowiremock.URLPathTemplate("/keys/signing/{kid}/revoke")).WithPathParam(
		"kid",
		gowiremock.Matching("kid"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
