...
package jobs_users_exports_test
...
func TestJobsUsersExportsCreateWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/jobs/users-exports")).WithBodyPattern(gowiremock.MatchesJsonSchema(`{
		...
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"status": "status", "type": "type", "created_at": "created_at", "id": "id", "connection_id": "connection_id", "format": "json", "limit": 1, "fields": []interface{}{map[string]interface{}{"name": "name", "export_as": "export_as"}}},
	...
	_, invocationErr := client.Jobs.UsersExports.Create(
		context.TODO(),
		request,
	)
