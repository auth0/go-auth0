...
func TestUsersRefreshTokenListWithWireMock(
	...
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/users/{user_id}/refresh-tokens")).WithPathParam(
		"user_id",
		gowiremock.Matching("user_id"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"tokens": []interface{}{map[string]interface{}{"id": "id", "user_id": "user_id", "created_at": "2024-01-15T09:30:00Z", "idle_expires_at": "2024-01-15T09:30:00Z", "expires_at": "2024-01-15T09:30:00Z", "client_id": "client_id", "session_id": "session_id", "rotating": true, "resource_servers": []interface{}{map[string]interface{}{}}, "last_exchanged_at": "2024-01-15T09:30:00Z"}}, "next": "next"},
...
func TestUsersRefreshTokenDeleteWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Delete(gowiremock.URLPathTemplate("/users/{user_id}/refresh-tokens")).WithPathParam(
		"user_id",
		gowiremock.Matching("user_id"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
