...
func TestUsersConnectedAccountsListWithWireMock(
	...
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/users/{id}/connected-accounts")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"connected_accounts": []interface{}{map[string]interface{}{"id": "id", "connection": "connection", "connection_id": "connection_id", "strategy": "strategy", "access_type": "offline", "scopes": []interface{}{"scopes"}, "created_at": "2024-01-15T09:30:00Z", "expires_at": "2024-01-15T09:30:00Z"}}, "next": "next"},
