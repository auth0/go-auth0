...
func (c *Client) List(
...
) (*core.Page[*int, *management.UserAuthenticationMethod], error) {
	...
	readPageResponse := func(response *management.ListUserAuthenticationMethodsOffsetPaginatedResponseContent) *core.PageResponse[*int, *management.UserAuthenticationMethod] {
		next += 1
		results := response.Authenticators
		return &core.PageResponse[*int, *management.UserAuthenticationMethod]{
			Next:    &next,
			Results: results,
		}
	}
