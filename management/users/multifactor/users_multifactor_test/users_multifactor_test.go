...
func TestUsersMultifactorInvalidateRememberBrowserWithWireMock(
	...
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/users/{id}/multifactor/actions/invalidate-remember-browser")).WithPathParam(
		"id",
...
func TestUsersMultifactorDeleteProviderWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Delete(gowiremock.URLPathTemplate("/users/{id}/multifactor/{provider}")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
	).WithPathParam(
		"provider",
		gowiremock.Matching("duo"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{},
		).WithStatus(http.StatusOK),
