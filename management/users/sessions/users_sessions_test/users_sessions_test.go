...
func TestUsersSessionsListWithWireMock(
	...
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/users/{user_id}/sessions")).WithPathParam(
		"user_id",
		gowiremock.Matching("user_id"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"sessions": []interface{}{map[string]interface{}{"id": "id", "user_id": "user_id", "created_at": "2024-01-15T09:30:00Z", "updated_at": "2024-01-15T09:30:00Z", "authenticated_at": "2024-01-15T09:30:00Z", "idle_expires_at": "2024-01-15T09:30:00Z", "expires_at": "2024-01-15T09:30:00Z", "last_interacted_at": "2024-01-15T09:30:00Z", "clients": []interface{}{map[string]interface{}{}}, "session_metadata": map[string]interface{}{"key": "value"}}}, "next": "next"},
...
func TestUsersSessionsDeleteWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Delete(gowiremock.URLPathTemplate("/users/{user_id}/sessions")).WithPathParam(
		"user_id",
		gowiremock.Matching("user_id"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
