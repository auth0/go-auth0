...
func TestUsersIdentitiesLinkWithWireMock(
	...
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/users/{id}/identities")).WithPathParam(
		"id",
		...
		gowiremock.NewResponse().WithJSONBody(
			[]interface{}{map[string]interface{}{"connection": "connection", "user_id": "user_id", "provider": "provider", "profileData": map[string]interface{}{"email": "email", "email_verified": true, "name": "name", "username": "username", "given_name": "given_name", "phone_number": "phone_number", "phone_verified": true, "family_name": "family_name"}, "isSocial": true, "access_token": "access_token", "access_token_secret": "access_token_secret", "refresh_token": "refresh_token"}},
...
func TestUsersIdentitiesDeleteWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Delete(gowiremock.URLPathTemplate("/users/{id}/identities/{provider}/{user_id}")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
	).WithPathParam(
		"provider",
		gowiremock.Matching("ad"),
	).WithPathParam(
		"user_id",
		gowiremock.Matching("user_id"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			[]interface{}{map[string]interface{}{"connection": "connection", "user_id": "user_id", "provider": "provider", "isSocial": true, "access_token": "access_token", "access_token_secret": "access_token_secret", "refresh_token": "refresh_token", "profileData": map[string]interface{}{"email": "email", "email_verified": true, "name": "name", "username": "username", "given_name": "given_name", "phone_number": "phone_number", "phone_verified": true, "family_name": "family_name"}}},
		).WithStatus(http.StatusOK),
	)
	err := WireMockClient.StubFor(stub)
	require.NoError(t, err, "Failed to create WireMock stub")
