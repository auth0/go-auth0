...
func (c *Client) List(
...
) (*core.Page[*int, *management.Organization], error) {
	...
	readPageResponse := func(response *management.ListUserOrganizationsOffsetPaginatedResponseContent) *core.PageResponse[*int, *management.Organization] {
		next += 1
		results := response.Organizations
		return &core.PageResponse[*int, *management.Organization]{
			Next:    &next,
			Results: results,
		}
	}
