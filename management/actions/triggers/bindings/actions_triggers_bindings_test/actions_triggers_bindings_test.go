...
func TestActionsTriggersBindingsListWithWireMock(
	...
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/actions/triggers/{triggerId}/bindings")).WithPathParam(
		"triggerId",
		gowiremock.Matching("triggerId"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"total": 1.1, "page": 1.1, "per_page": 1.1, "bindings": []interface{}{map[string]interface{}{"id": "id", "trigger_id": "trigger_id", "display_name": "display_name", "created_at": "2024-01-15T09:30:00Z", "updated_at": "2024-01-15T09:30:00Z"}}},
...
func TestActionsTriggersBindingsUpdateManyWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Patch(gowiremock.URLPathTemplate("/actions/triggers/{triggerId}/bindings")).WithPathParam(
		"triggerId",
		gowiremock.Matching("triggerId"),
		...
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"bindings": []interface{}{map[string]interface{}{"id": "id", "trigger_id": "trigger_id", "display_name": "display_name", "created_at": "2024-01-15T09:30:00Z", "updated_at": "2024-01-15T09:30:00Z"}}},
		).WithStatus(http.StatusOK),
	)
	err := WireMockClient.StubFor(stub)
	require.NoError(t, err, "Failed to create WireMock stub")
