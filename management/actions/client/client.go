...
func (c *Client) List(
...
) (*core.Page[*int, *management.Action], error) {
	...
	readPageResponse := func(response *management.ListActionsPaginatedResponseContent) *core.PageResponse[*int, *management.Action] {
		next += 1
		results := response.Actions
		return &core.PageResponse[*int, *management.Action]{
			Next:    &next,
			Results: results,
		}
	}
