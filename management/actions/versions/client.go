...
func (c *Client) List(
...
) (*core.Page[*int, *management.ActionVersion], error) {
	...
	readPageResponse := func(response *management.ListActionVersionsPaginatedResponseContent) *core.PageResponse[*int, *management.ActionVersion] {
		next += 1
		results := response.Versions
		return &core.PageResponse[*int, *management.ActionVersion]{
			Next:    &next,
			Results: results,
		}
	}
