...
func TestHooksListWithWireMock(
	...
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/hooks")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"start": 1.1, "limit": 1.1, "total": 1.1, "hooks": []interface{}{map[string]interface{}{"triggerId": "triggerId", "id": "id", "name": "name", "enabled": true, "script": "script", "dependencies": map[string]interface{}{"key": "value"}}}},
...
func TestHooksCreateWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/hooks")).WithBodyPattern(gowiremock.MatchesJsonSchema(`{
                    "$schema": "https://json-schema.org/draft/2020-12/schema",
                    "type": "object",
                    "required": ["name", "script", "triggerId"],
                    "properties": {
                        "name": {"type": "string"}, "script": {"type": "string"}, "triggerId": {"type": "string"}
                    },
                    "additionalProperties": true
                }`, "V202012")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"triggerId": "triggerId", "id": "id", "name": "name", "enabled": true, "script": "script", "dependencies": map[string]interface{}{"key": "value"}},
		).WithStatus(http.StatusOK),
	)
	err := WireMockClient.StubFor(stub)
	require.NoError(t, err, "Failed to create WireMock stub")
...
func TestHooksGetWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/hooks/{id}")).WithPathParam(
		"id",
...
func TestHooksDeleteWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Delete(gowiremock.URLPathTemplate("/hooks/{id}")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
...
func TestHooksUpdateWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Patch(gowiremock.URLPathTemplate("/hooks/{id}")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
