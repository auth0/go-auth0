...
func TestLogsListWithWireMock(
	...
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/logs")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"start": 1.1, "limit": 1.1, "length": 1.1, "total": 1.1, "logs": []interface{}{map[string]interface{}{"date": "date", "type": "type", "description": "description", "connection": "connection", "connection_id": "connection_id", "client_id": "client_id", "client_name": "client_name", "ip": "ip", "hostname": "hostname", "user_id": "user_id", "user_name": "user_name", "audience": "audience", "scope": "scope", "strategy": "strategy", "strategy_type": "strategy_type", "log_id": "log_id", "isMobile": true, "details": map[string]interface{}{"key": "value"}, "user_agent": "user_agent"}}},
...
func TestLogsGetWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/logs/{id}")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"date": "date", "type": "type", "description": "description", "connection": "connection", "connection_id": "connection_id", "client_id": "client_id", "client_name": "client_name", "ip": "ip", "hostname": "hostname", "user_id": "user_id", "user_name": "user_name", "audience": "audience", "scope": "scope", "strategy": "strategy", "strategy_type": "strategy_type", "log_id": "log_id", "isMobile": true, "details": map[string]interface{}{"key": "value"}, "user_agent": "user_agent", "security_context": map[string]interface{}{"ja3": "ja3", "ja4": "ja4"}, "location_info": map[string]interface{}{"country_code": "country_code", "country_code3": "country_code3", "country_name": "country_name", "city_name": "city_name", "latitude": "latitude", "longitude": "longitude", "time_zone": "time_zone", "continent_code": "continent_code"}},
		).WithStatus(http.StatusOK),
	)
	err := WireMockClient.StubFor(stub)
	require.NoError(t, err, "Failed to create WireMock stub")
