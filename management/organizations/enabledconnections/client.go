...
func (c *Client) List(
...
) (*core.Page[*int, *management.OrganizationConnection], error) {
	...
	readPageResponse := func(response *management.ListOrganizationConnectionsOffsetPaginatedResponseContent) *core.PageResponse[*int, *management.OrganizationConnection] {
		next += 1
		results := response.EnabledConnections
		return &core.PageResponse[*int, *management.OrganizationConnection]{
			Next:    &next,
			Results: results,
		}
	}
