...
func TestOrganizationsClientGrantsListWithWireMock(
	...
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/organizations/{id}/client-grants")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"start": 1.1, "limit": 1.1, "total": 1.1, "client_grants": []interface{}{map[string]interface{}{"id": "id", "client_id": "client_id", "audience": "audience", "scope": []interface{}{"scope"}, "organization_usage": "deny", "allow_any_organization": true}}},
...
func TestOrganizationsClientGrantsCreateWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/organizations/{id}/client-grants")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
		...
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"id": "id", "client_id": "client_id", "audience": "audience", "scope": []interface{}{"scope"}, "organization_usage": "deny", "allow_any_organization": true},
		).WithStatus(http.StatusOK),
	)
	err := WireMockClient.StubFor(stub)
	require.NoError(t, err, "Failed to create WireMock stub")
...
func TestOrganizationsClientGrantsDeleteWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Delete(gowiremock.URLPathTemplate("/organizations/{id}/client-grants/{grant_id}")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
	).WithPathParam(
		"grant_id",
