...
func (c *Client) List(
...
) (*core.Page[*int, *management.OrganizationInvitation], error) {
	...
	readPageResponse := func(response *management.ListOrganizationInvitationsOffsetPaginatedResponseContent) *core.PageResponse[*int, *management.OrganizationInvitation] {
		next += 1
		results := response.Invitations
		return &core.PageResponse[*int, *management.OrganizationInvitation]{
			Next:    &next,
			Results: results,
		}
	}
