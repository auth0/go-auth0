...
func (c *Client) List(
...
) (*core.Page[*int, *management.Role], error) {
	...
	readPageResponse := func(response *management.ListOrganizationMemberRolesOffsetPaginatedResponseContent) *core.PageResponse[*int, *management.Role] {
		next += 1
		results := response.Roles
		return &core.PageResponse[*int, *management.Role]{
			Next:    &next,
			Results: results,
		}
	}
