...
func (c *Client) List(
...
) (*core.Page[*int, *management.NetworkACLsResponseContent], error) {
	...
	readPageResponse := func(response *management.ListNetworkACLsOffsetPaginatedResponseContent) *core.PageResponse[*int, *management.NetworkACLsResponseContent] {
		next += 1
		results := response.NetworkACLs
		return &core.PageResponse[*int, *management.NetworkACLsResponseContent]{
			Next:    &next,
			Results: results,
		}
	}
