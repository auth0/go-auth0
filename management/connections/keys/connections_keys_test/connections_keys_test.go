...
func TestConnectionsKeysGetWithWireMock(
	t *testing.T,
) {
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/connections/{id}/keys")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			[]interface{}{map[string]interface{}{"kid": "kid", "cert": "cert", "pkcs": "pkcs", "current": true, "next": true, "previous": true, "current_since": "current_since", "fingerprint": "fingerprint", "thumbprint": "thumbprint", "algorithm": "algorithm", "key_use": "encryption", "subject_dn": "subject_dn"}},
	...
	_, invocationErr := client.Connections.Keys.Get(
		context.TODO(),
		"id",
	)
...
func TestConnectionsKeysRotateWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/connections/{id}/keys/rotate")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
	).WithBodyPattern(gowiremock.MatchesJsonSchema("{}", "V202012")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"kid": "kid", "cert": "cert", "pkcs": "pkcs", "next": true, "fingerprint": "fingerprint", "thumbprint": "thumbprint", "algorithm": "algorithm", "key_use": "encryption", "subject_dn": "subject_dn"},
		).WithStatus(http.StatusOK),
	)
	err := WireMockClient.StubFor(stub)
	require.NoError(t, err, "Failed to create WireMock stub")
	...
	_, invocationErr := client.Connections.Keys.Rotate(
		context.TODO(),
		"id",
		request,
	)
