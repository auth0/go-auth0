// Code generated by Fern. DO NOT EDIT.

package client

import (
	context "context"
	management "github.com/auth0/go-auth0/v2/management"
	synchronizations "github.com/auth0/go-auth0/v2/management/connections/directoryprovisioning/synchronizations"
	core "github.com/auth0/go-auth0/v2/management/core"
	internal "github.com/auth0/go-auth0/v2/management/internal"
	option "github.com/auth0/go-auth0/v2/management/option"
	http "net/http"
)

type Client struct {
	WithRawResponse  *RawClient
	Synchronizations *synchronizations.Client

	options *core.RequestOptions
	baseURL string
	caller  *internal.Caller
}

func NewClient(options *core.RequestOptions) *Client {
	return &Client{
		Synchronizations: synchronizations.NewClient(options),
		WithRawResponse:  NewRawClient(options),
		options:          options,
		baseURL:          options.BaseURL,
		caller: internal.NewCaller(
			&internal.CallerParams{
				Client:      options.HTTPClient,
				MaxAttempts: options.MaxAttempts,
			},
		),
	}
}

// Retrieve a list of directory provisioning configurations of a tenant.
func (c *Client) List(
	ctx context.Context,
	request *management.ListDirectoryProvisioningsRequestParameters,
	opts ...option.RequestOption,
) (*core.Page[*string, *management.DirectoryProvisioning, *management.ListDirectoryProvisioningsResponseContent], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		c.baseURL,
		"https://%7BTENANT%7D.auth0.com/api/v2",
	)
	endpointURL := baseURL + "/connections-directory-provisionings"
	queryParams, err := internal.QueryValuesWithDefaults(
		request,
		map[string]any{
			"take": 50,
		},
	)
	if err != nil {
		return nil, err
	}
	headers := internal.MergeHeaders(
		c.options.ToHeader(),
		options.ToHeader(),
	)
	prepareCall := func(pageRequest *core.PageRequest[*string]) *internal.CallParams {
		if pageRequest.Cursor != nil {
			queryParams.Set("from", *pageRequest.Cursor)
		}
		nextURL := endpointURL
		if len(queryParams) > 0 {
			nextURL += "?" + queryParams.Encode()
		}
		return &internal.CallParams{
			URL:             nextURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        pageRequest.Response,
			ErrorDecoder:    internal.NewErrorDecoder(management.ErrorCodes),
		}
	}
	readPageResponse := func(response *management.ListDirectoryProvisioningsResponseContent) *core.PageResponse[*string, *management.DirectoryProvisioning, *management.ListDirectoryProvisioningsResponseContent] {
		var zeroValue *string
		next := response.Next
		results := response.DirectoryProvisionings
		return &core.PageResponse[*string, *management.DirectoryProvisioning, *management.ListDirectoryProvisioningsResponseContent]{
			Results:  results,
			Response: response,
			Next:     next,
			Done:     next == zeroValue,
		}
	}
	pager := internal.NewCursorPager(
		c.caller,
		prepareCall,
		readPageResponse,
	)
	return pager.GetPage(ctx, request.From)
}

// Retrieve the directory provisioning configuration of a connection.
func (c *Client) Get(
	ctx context.Context,
	// The id of the connection to retrieve its directory provisioning configuration
	id string,
	opts ...option.RequestOption,
) (*management.GetDirectoryProvisioningResponseContent, error) {
	response, err := c.WithRawResponse.Get(
		ctx,
		id,
		opts...,
	)
	if err != nil {
		return nil, err
	}
	return response.Body, nil
}

// Create a directory provisioning configuration for a connection.
func (c *Client) Create(
	ctx context.Context,
	// The id of the connection to create its directory provisioning configuration
	id string,
	request *management.CreateDirectoryProvisioningRequestContent,
	opts ...option.RequestOption,
) (*management.CreateDirectoryProvisioningResponseContent, error) {
	response, err := c.WithRawResponse.Create(
		ctx,
		id,
		request,
		opts...,
	)
	if err != nil {
		return nil, err
	}
	return response.Body, nil
}

// Delete the directory provisioning configuration of a connection.
func (c *Client) Delete(
	ctx context.Context,
	// The id of the connection to delete its directory provisioning configuration
	id string,
	opts ...option.RequestOption,
) error {
	_, err := c.WithRawResponse.Delete(
		ctx,
		id,
		opts...,
	)
	if err != nil {
		return err
	}
	return nil
}

// Update the directory provisioning configuration of a connection.
func (c *Client) Update(
	ctx context.Context,
	// The id of the connection to create its directory provisioning configuration
	id string,
	request *management.UpdateDirectoryProvisioningRequestContent,
	opts ...option.RequestOption,
) (*management.UpdateDirectoryProvisioningResponseContent, error) {
	response, err := c.WithRawResponse.Update(
		ctx,
		id,
		request,
		opts...,
	)
	if err != nil {
		return nil, err
	}
	return response.Body, nil
}

// Retrieve the directory provisioning default attribute mapping of a connection.
func (c *Client) GetDefaultMapping(
	ctx context.Context,
	// The id of the connection to retrieve its directory provisioning configuration
	id string,
	opts ...option.RequestOption,
) (*management.GetDirectoryProvisioningDefaultMappingResponseContent, error) {
	response, err := c.WithRawResponse.GetDefaultMapping(
		ctx,
		id,
		opts...,
	)
	if err != nil {
		return nil, err
	}
	return response.Body, nil
}
