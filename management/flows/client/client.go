...
func (c *Client) List(
...
) (*core.Page[*int, *management.FlowSummary], error) {
	...
	readPageResponse := func(response *management.ListFlowsOffsetPaginatedResponseContent) *core.PageResponse[*int, *management.FlowSummary] {
		next += 1
		results := response.Flows
		return &core.PageResponse[*int, *management.FlowSummary]{
			Next:    &next,
			Results: results,
		}
	}
