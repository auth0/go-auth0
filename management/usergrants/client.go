...
func (c *Client) List(
...
) (*core.Page[*int, *management.UserGrant], error) {
	...
	readPageResponse := func(response *management.ListUserGrantsOffsetPaginatedResponseContent) *core.PageResponse[*int, *management.UserGrant] {
		next += 1
		results := response.Grants
		return &core.PageResponse[*int, *management.UserGrant]{
			Next:    &next,
			Results: results,
		}
	}
