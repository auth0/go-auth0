...
func (c *Client) List(
...
) (*core.Page[*int, *management.Rule], error) {
	...
	readPageResponse := func(response *management.ListRulesOffsetPaginatedResponseContent) *core.PageResponse[*int, *management.Rule] {
		next += 1
		results := response.Rules
		return &core.PageResponse[*int, *management.Rule]{
			Next:    &next,
			Results: results,
		}
	}
