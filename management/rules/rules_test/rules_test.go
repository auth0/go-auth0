...
func TestRulesListWithWireMock(
	...
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/rules")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"start": 1.1, "limit": 1.1, "total": 1.1, "rules": []interface{}{map[string]interface{}{"name": "name", "id": "id", "enabled": true, "script": "script", "order": 1.1, "stage": "stage"}}},
...
func TestRulesCreateWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/rules")).WithBodyPattern(gowiremock.MatchesJsonSchema(`{
		...
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"name": "name", "id": "id", "enabled": true, "script": "script", "order": 1.1, "stage": "stage"},
		).WithStatus(http.StatusOK),
	)
	err := WireMockClient.StubFor(stub)
	require.NoError(t, err, "Failed to create WireMock stub")
...
func TestRulesGetWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/rules/{id}")).WithPathParam(
		"id",
...
func TestRulesDeleteWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Delete(gowiremock.URLPathTemplate("/rules/{id}")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
...
func TestRulesUpdateWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Patch(gowiremock.URLPathTemplate("/rules/{id}")).WithPathParam(
		"id",
		gowiremock.Matching("id"),
