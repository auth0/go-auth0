...
func (c *Client) List(
...
) (*core.Page[*int, *management.ResourceServer], error) {
	...
	readPageResponse := func(response *management.ListResourceServerOffsetPaginatedResponseContent) *core.PageResponse[*int, *management.ResourceServer] {
		next += 1
		results := response.ResourceServers
		return &core.PageResponse[*int, *management.ResourceServer]{
			Next:    &next,
			Results: results,
		}
	}
