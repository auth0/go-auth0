...
func TestPromptsGetSettingsWithWireMock(
	...
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/prompts")).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"universal_login_experience": "new", "identifier_first": true, "webauthn_platform_first_factor": true},
...
func TestPromptsUpdateSettingsWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Patch(gowiremock.URLPathTemplate("/prompts")).WithBodyPattern(gowiremock.MatchesJsonSchema(`{
