...
func (c *Client) List(
...
) (*core.Page[*int, *management.AculResponseContent], error) {
	...
	readPageResponse := func(response *management.ListAculsOffsetPaginatedResponseContent) *core.PageResponse[*int, *management.AculResponseContent] {
		next += 1
		results := response.Configs
		return &core.PageResponse[*int, *management.AculResponseContent]{
			Next:    &next,
			Results: results,
		}
	}
