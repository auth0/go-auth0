...
func TestEmailTemplatesCreateWithWireMock(
	...
	stub := gowiremock.Post(gowiremock.URLPathTemplate("/email-templates")).WithBodyPattern(gowiremock.MatchesJsonSchema(`{
		...
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"template": "verify_email", "body": "body", "from": "from", "resultUrl": "resultUrl", "subject": "subject", "syntax": "syntax", "urlLifetimeInSeconds": 1.1, "includeEmailInRedirect": true, "enabled": true},
...
func TestEmailTemplatesGetWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Get(gowiremock.URLPathTemplate("/email-templates/{templateName}")).WithPathParam(
		"templateName",
		gowiremock.Matching("verify_email"),
	).WillReturnResponse(
		gowiremock.NewResponse().WithJSONBody(
			map[string]interface{}{"template": "verify_email", "body": "body", "from": "from", "resultUrl": "resultUrl", "subject": "subject", "syntax": "syntax", "urlLifetimeInSeconds": 1.1, "includeEmailInRedirect": true, "enabled": true},
		).WithStatus(http.StatusOK),
...
func TestEmailTemplatesSetWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Put(gowiremock.URLPathTemplate("/email-templates/{templateName}")).WithPathParam(
		"templateName",
		gowiremock.Matching("verify_email"),
...
func TestEmailTemplatesUpdateWithWireMock(
	...
	// wiremock client and server initialized in shared main_test.go
	defer WireMockClient.Reset()
	stub := gowiremock.Patch(gowiremock.URLPathTemplate("/email-templates/{templateName}")).WithPathParam(
		"templateName",
		gowiremock.Matching("verify_email"),
	).WithBodyPattern(gowiremock.MatchesJsonSchema(`{
                    "$schema": "https://json-schema.org/draft/2020-12/schema",
